    ...
    <script>
        $(document).ready(function(){
            $("#btn_Create").click(function(){
                window.location.href="${ctx}/category/create";
            }) ;
            ...
            $(".cla_edit").bind("click",function () {
                window.location.href="${ctx}/category/update/${category.id}";
            })
            $("#btn_Delete").bind("click",function () {
                ...
                if(count<1){
                    ...
                    if (confirm("确定要删除所选项吗？")){
                        $("#listFrm").attr({action:"${ctx}/category/delete", method:"POST"})
                        $("#listFrm").submit();
                    }
                }
...
<div class="container">
    ...
    <form id="listFrm">
        <table id="contentTable" class="table table-striped table-hover">
            ...
            <tbody>
            <c:forEach items="${categorys.content}" var="category" varStatus="idxStatus">
                <tr>
                    <td><input type="checkbox" name="chkIds" value="${category.id}"></td>
                    <td>${idxStatus.index+1}</td>
                    <td>${category.sort}</td>
                    <td>${category.sort1}</td>
                    <td>${category.sort2}</td>
                    <td>${category.name}</td>
                    <td>${category.status}</td>
                    <td>
                        <a href="${ctx}/category/update/${category.id}">编辑</a>
                            <%--                    <a href="#" class="cla_edit" id="${dict.id}">编辑</a>--%>
                        |
                        <a href="#" class="cla_delete" id="${category.id}">删除</a>
                    </td>
                </tr>
            </c:forEach>
            </tbody>
        </table>
        <tags:pagination page="${categorys}" paginationSize="${PAGE_SIZE}"/>
    </form>
</div>
